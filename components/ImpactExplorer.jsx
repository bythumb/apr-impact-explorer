...
const CATEGORIES = {
  ...
  digital: {
    ...
    kind: "problem",
    commentary: [
      "> 2005: 353M tracks. iTunes Store grows fast — but revenue per $0.99 track is a fraction of a $15 CD.",
      "> 2006: 582M tracks. Up 65%. Album-as-product is dying; volume can't replace dollars.",
      "> 2007: 844M tracks. Amazon MP3 launches DRM-free. Track count up, revenue per track flat.",
      "> 2008: 1.07B tracks. Single-track sales exceed full albums in volume — the bridge is collapsing.",
      "> 2009: 1.16B tracks. The 'digital savior' grew 228% and still cannot offset CD losses. The math does not work.",
    ],
  },
  youtube: {
...
function xPos(i) {
  return PAD.l + ((i + 0.5) / YEARS.length) * PLOT_W;
}

function drawAxes(svg, cat) {
  const { yMin, yMax } = cat;
  for (let i = 0; i <= 5; i++) {
...
function renderBar(svg, cat, globalProgress) {
  svg.innerHTML = "";
  drawAxes(svg, cat);
...
function renderBarH(svg, cat, globalProgress) {
  svg.innerHTML = "";
...
function renderLine(svg, cat, upTo, animProgress) {
  svg.innerHTML = "";
  drawAxes(svg, cat);
  YEARS.forEach((yr, i) => {
    const cx = xPos(i);
...
function renderArea(svg, cat, upTo, animProgress) {
  svg.innerHTML = "";
  drawAxes(svg, cat);
  YEARS.forEach((yr, i) => {
    const cx = xPos(i);
...
function renderPie(svg, cat, upTo, animProgress) {
  svg.innerHTML = "";
...
function renderFor(svg, cat, upTo, animProgress, globalProgress) {
  switch (cat.chartType) {
    case "bar": renderBar(svg, cat, globalProgress); break;
    case "bar-h": renderBarH(svg, cat, globalProgress); break;
    case "line": renderLine(svg, cat, upTo, animProgress); break;
    case "area": renderArea(svg, cat, upTo, animProgress); break;
    case "pie": renderPie(svg, cat, upTo, animProgress); break;
  }
}

function makeAnimator(refs) {
  const {
    svg, phaseBar, commentary, recBox, recText,
    statusLine, metricLabel, dataTable, totalLabel, totalValue,
  } = refs;
  ...
  function updatePhaseBar(progress) {
    if (!phaseBarFills.length) return;
  ...
  function updateSidebar(cat, upTo) {
    if (!metricLabel || !dataTable || !totalLabel || !totalValue) return;
  ...
  function playAnimation(cat, key) {
    if (animTimer) cancelAnimationFrame(animTimer);

    buildPhaseBar();
    updateSidebar(cat, 0);
    updatePhaseBar(0);
    renderFor(svg, cat, 0, 0, 0);
    if (commentary) commentary.textContent = cat.commentary[0];
    ...
    function frame(now) {
      ...
      renderFor(svg, cat, upTo, animProgress, eased);
      updatePhaseBar(eased);

      const cIdx = Math.min(N - 1, Math.floor(scaled));
      if (cIdx !== lastCommentaryIdx) {
        if (commentary) commentary.textContent = cat.commentary[cIdx];
      ...
      if (linear < 1) {
        ...
        updateSidebar(cat, N);
        if (statusLine) statusLine.textContent = "> COMPLETE";
        if (recText) recText.textContent = RECOMMENDATIONS[key] || "";
...
export default function ImpactExplorer() {
  ...
  useEffect(() => {
    ...
    Object.entries(CATEGORIES).forEach(([key, cat]) => {
      const row = buildWordRow(key, cat, idxCounter++);
    ...
    const desktopAnimator = makeAnimator({
      svg: $("chart-svg"),
      phaseBar: $("phase-bar"),
    ...
    function openCategory(key) {
      ...
      setActiveMetric(key);
      desktopAnimator.playAnimation(cat, key);
    }

    const onBack = () => {
      desktopAnimator.cancel();
    ...
    Object.entries(CATEGORIES).forEach(([key, cat], i) => {
      ...
      const tag = cat.kind === "solution" ? "OPPORTUNITY" : "PROBLEM";
      section.innerHTML = `
        <div class="mm-tag">&gt; ${tag}</div>
        <div class="mm-header">
          <span class="mm-idx">[${String(idx).padStart(2, "0")}]</span>
          <span class="mm-name">${cat.word}</span>
        </div>
        <div class="mm-subtitle">${cat.subtitle}</div>
        <svg class="mm-svg" viewBox="0 0 ${W} ${H}"></svg>
        <div class="mm-phase-bar"></div>
        <div class="mm-commentary">&gt; SCROLL TO REVEAL</div>
        <div class="mm-rec" style="display:none">
          <div class="mm-rec-label">&gt; RECOMMENDATION</div>
          <div class="mm-rec-text"></div>
        </div>
      `;
      mobileView.appendChild(section);

      const animator = makeAnimator({
        svg: section.querySelector(".mm-svg"),
        phaseBar: section.querySelector(".mm-phase-bar"),
        commentary: section.querySelector(".mm-commentary"),
        recBox: section.querySelector(".mm-rec"),
        recText: section.querySelector(".mm-rec-text"),
        statusLine: null,
        metricLabel: null,
        dataTable: null,
        totalLabel: null,
      ...
      mobileSections[key] = { section, animator, played: false, cat };
    });
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const m = mobileSections[key];
            if (m && !m.played) {
              m.played = true;
              m.animator.playAnimation(m.cat, key);
            }
          }
        });
      },
    ...
    Object.values(mobileSections).forEach((m) => observer.observe(m.section));

    return () => {
      clearInterval(clockId);
      desktopAnimator.cancel();
      Object.values(mobileSections).forEach((m) => m.animator.cancel());
      observer.disconnect();
  ...
  return (
    ...
    >
      <h2 style={{ position: "absolute", left: "-9999px" }}>
        APR Records impact explorer with real RIAA data 2005 to 2009.
      </h2>
      ...
      <div id="desktop-view">
        ...
        <div id="chart-view" style={{ display: "none", paddingTop: "1rem" }}>
          <div
            style={{
              display: "flex",
              justifyContent: "space-between",
